...
clientDao.addLocationToClient = function (location) {
    ...
    clientModel.findOneAndUpdate({ _id: location.client }, {
        $addToSet: {
...
clientDao.updateClient = function (obj) {
    ...
    if (req.body.name) updatedClient['name'] = req.body.name;
    if (req.body.email) updatedClient['email'] = req.body.email;
    if (req.body.password) updatedClient['password'] = req.body.password;
